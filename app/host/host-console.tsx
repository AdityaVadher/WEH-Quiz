...
type HostGame = {
  ...
  guesses: Array<{ playerId: string; playerName: string; playerEmail: string | null; guess: string; clueNumber: number; correct: boolean; points: number }>;
  lastQuestionResults: null | {
    question: number;
    correctCount: number;
    answeredCount: number;
    firstFive: Array<{ playerName: string; playerEmail: string | null; clueNumber: number; correct: boolean; points: number; position: number }>;
  };
};
...
function LastQuestionResults({ results }: { results: HostGame["lastQuestionResults"] }) {
  return <section className="dark-card">
    <div className="flex items-start justify-between gap-3">
      <div><p className="eyebrow text-[#8298b1]">Last question results</p><h2 className="mt-1 font-display text-2xl">{results ? `Question ${results.question}` : "Waiting for Question 1"}</h2></div>
      {results && <div className="rounded-xl bg-[#1c7568]/20 px-3 py-2 text-center text-[#71d3b4]"><span className="block font-display text-2xl leading-none">{results.correctCount}</span><span className="text-[10px] font-black uppercase tracking-wider">Correct</span></div>}
    </div>
    {results ? <>
      <p className="mt-3 text-sm text-[#8ca1b8]">{results.correctCount} of {results.answeredCount} answers were correct.</p>
      <div className="mt-4 space-y-2"><p className="text-xs font-black uppercase tracking-[.14em] text-[#70869f]">First five answers</p>{results.firstFive.length ? results.firstFive.map((entry) => <div key={`${entry.position}-${entry.playerName}`} className="flex items-center gap-3 rounded-xl bg-white/5 px-3 py-2.5"><span className="w-5 font-black text-[#ffd34e]">{entry.position}</span><span className="min-w-0 flex-1"><span className="block truncate font-bold">{entry.playerName}</span><span className="block truncate text-xs text-[#70869f]">Clue {entry.clueNumber} · {entry.playerEmail ?? "No email"}</span></span><span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${entry.correct ? "bg-[#1c7568]/20 text-[#71d3b4]" : "bg-[#ff5c4d]/15 text-[#ff9b92]"}`}>{entry.correct ? `Correct +${entry.points}` : "Incorrect"}</span></div>) : <p className="rounded-xl border border-dashed border-white/10 p-4 text-center text-sm text-[#71869e]">No answers were submitted</p>}</div>
    </> : <p className="mt-3 text-sm text-[#8ca1b8]">Results will appear after the first answer is revealed.</p>}
  </section>;
}
